...
This component combines several motion techniques:

* 3D card tilt using `rotateX` and `rotateY`.
* Subtle card movement using `translateX` and `translateY`.
* A cursor-following spotlight using a radial gradient.
* Spring animations for smoother, more natural motion.
* Motion values and transforms to map cursor position into animation values.
